...
static gchar *ws_server_addr = "127.0.0.1";
...
guint64 RTCPNTPtime, ntpns, rtpdiff, ntpnschange;
guint32 RTCPRTPtime;
long int ns;
  ...
  if (gst_rtcp_buffer_validate(info))
  {
    ...
    while (more)
    {
      if (gst_rtcp_packet_get_type(&packet) == GST_RTCP_TYPE_SR)
      {
        ...
        printf("\n NTP %" G_GUINT64_FORMAT " RTCPRTP %d ssrc %d\n ", ntpns, RTCPRTPtime, ssrc);
        ...
        gpointer user_data)
{
  ...
  if (RTCPRTPtime != 0)
    if (gst_rtp_buffer_get_timestamp(&rtp) >= RTCPRTPtime)
    {
      g_print("changed to new NTP changed");
      ntpnschange = ntpns;
      RTCPRTPtime = 0;
    }
  if (rtptime != gst_rtp_buffer_get_timestamp(&rtp))
  {
    rtpdiff = (((float)(gst_rtp_buffer_get_timestamp(&rtp) - rtptime) / 90000) * 1000000000);
    ntpnschange = rtpdiff + ntpnschange;
  }
  ...
  printf("%ld %ld %ld\n", all, rtpdiff, ntpnschange);

  gst_rtp_buffer_unmap(&rtp);

  // gst_pipeline_set_latency(gstpipline, pipelinedelay);
  //  GstMapInfo lsMap;
                   ...
                   GstElement *pipe)
{
  ...
  gst_pad_add_probe(pad, GST_PAD_PROBE_TYPE_BUFFER,
                    (GstPadProbeCallback)get_rtp, NULL, NULL);
...
{
  GstElement *pipe;
  pipe = gst_parse_launch("webrtcbin name=webrtcbin stun-server=stun://" STUN_SERVER " "
  ...
  g_signal_emit_by_name(webrtc, "add-transceiver", GST_WEBRTC_RTP_TRANSCEIVER_DIRECTION_RECVONLY, caps);
